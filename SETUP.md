...
`xfleet session-init` resolves repo names to absolute paths from the `repos` object in `~/.config/xfleet/config.json`:

```json
{
  "repos": {
    "oracle": "/absolute/path/to/oracle",
    "server": "/absolute/path/to/merlin-ai"
  }
}
```
